...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class SpeedHackDetector : ActDetectorBase
	{
		...
		internal const string FINAL_LOG_PREFIX = CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + COMPONENT_NAME + ": ";
		...
		private void Update()
		{
			...
			if (ticks - prevIntervalTicks >= intervalTicks)
			{
				...
				if (Mathf.Abs((vulnerableTicks - vulnerableTicksOnStart) - (ticks - ticksOnStart)) > THRESHOLD)
				{
					...
					{
#if DEBUG_ENABLED
						Debug.LogWarning(CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + "SpeedHackDetector: final detection!", this);
#endif
						OnCheatingDetected();
					}
					else
					{
#if DEBUG_ENABLED
						Debug.LogWarning(CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + "SpeedHackDetector: detection! Allowed false positives left: " + (maxFalsePositives - currentFalsePositives), this);
#endif
				...
				{
#if DEBUG_ENABLED
					Debug.LogWarning(CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + "SpeedHackDetector: success shot! Shots till cool down: " + (coolDown - currentCooldownShots), this);
#endif
					currentCooldownShots++;
					if (currentCooldownShots >= coolDown)
					{
#if DEBUG_ENABLED
						Debug.LogWarning(CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + "SpeedHackDetector: cool down!", this);
#endif
