...
namespace CodeStage.AntiCheat.ObscuredTypes
{
	...
	public static class ObscuredPrefs
	{
		...
		public static void ForceLockToDeviceInit()
		{
			...
			{
				Debug.LogWarning(CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + "ObscuredPrefs.ForceLockToDeviceInit() is called, but device ID is already obtained!");
			}
		}
		...
		private static string GetEncryptedPrefsString(string key, string encryptedKey)
		{
			...
			if (result == RAW_NOT_FOUND)
			{
				if (PlayerPrefs.HasKey(key))
				{
					Debug.LogWarning(CodeStage.AntiCheat.Common.Constants.LOG_PREFIX + "Are you trying to read regular PlayerPrefs data using ObscuredPrefs (key = " + key + ")?");
				}
			}
			return result;
		}
